...
export class ModelRuntime implements Models {
	...
	private readonly extensionProviders = new Map<string, { config: ProviderConfigInput; owner?: string }>();
	private readonly nativeExtensionProviders = new Map<string, { provider: Provider; owner?: string }>();
	...
	private recomposeProvider(providerId: string): void {
		const base = this.nativeExtensionProviders.get(providerId)?.provider ?? this.builtins.get(providerId);
		const extension = this.extensionProviders.get(providerId)?.config;
	...
	getRegisteredProviderConfig(providerId: string): ProviderConfigInput | undefined {
		return this.extensionProviders.get(providerId)?.config;
	}
	...
	getRegisteredNativeProvider(providerId: string): Provider | undefined {
		return this.nativeExtensionProviders.get(providerId)?.provider;
	}
	...
	getCompatibilityRequestConfig(model: Model<Api>): CompatibilityRequestConfig {
		return resolveCompatibilityRequestConfig(
			...
			this.extensionProviders.get(model.provider)?.config,
		);
	}
	...
	async getAuth(
		...
		const configuredHeaders = resolveConfiguredModelHeaders(
			...
			this.extensionProviders.get(providerOrModel.provider)?.config,
	...
	getProviderAuthStatus(providerId: string): AuthStatus {
		...
		const configured = configuredRequestAuthStatus(
			...
			this.extensionProviders.get(providerId)?.config,
		);
	...
	registerNativeProvider(provider: Provider, owner?: string): void {
		if (!provider.id.trim()) throw new Error("Provider id must not be empty.");
		this.extensionProviders.delete(provider.id);
		this.nativeExtensionProviders.set(provider.id, { provider, owner });
		this.recomposeProvider(provider.id);
		this.updateModelSnapshot();
		void this.refresh({ allowNetwork: false });
	}

	registerProvider(providerId: string, config: ProviderConfigInput, owner?: string): void {
		...
		const effective: ProviderConfigInput = { ...(previous?.config ?? {}) };
	...
	unregisterProvider(providerId: string, owner?: string): void {
		const configRegistration = this.extensionProviders.get(providerId);
		const nativeRegistration = this.nativeExtensionProviders.get(providerId);
		if (!configRegistration && !nativeRegistration) return;
		if (owner !== undefined && (configRegistration?.owner ?? nativeRegistration?.owner) !== owner) return;
	...
	/** Remove providers registered by extensions (owned registrations). */
	clearExtensionProviders(): void {
		const ownedProviderIds = new Set([
			...[...this.extensionProviders.entries()]
				.filter(([, state]) => state.owner !== undefined)
				.map(([providerId]) => providerId),
			...[...this.nativeExtensionProviders.entries()]
				.filter(([, state]) => state.owner !== undefined)
				.map(([providerId]) => providerId),
		]);
		if (ownedProviderIds.size === 0) return;
