...
export class ExtensionRunner {
	...
	private getRetryPolicyFn: ExtensionContextActions["getRetryPolicy"];
	...
	bindCore(
		...
		this.getRetryPolicyFn = contextActions.getRetryPolicy;
		...
		for (const { name, config, extensionPath } of this.runtime.pendingProviderRegistrations) {
			try {
				if (providerActions?.registerProvider) {
					...
					this.modelRegistry.registerProvider(name, config, extensionPath);
				}
			} catch (err) {
				this.emitError({
					extensionPath,
		...
		for (const { provider, extensionPath } of this.runtime.pendingNativeProviderRegistrations) {
			try {
				if (providerActions?.registerNativeProvider) {
					...
					this.modelRegistry.registerProvider(provider, extensionPath);
				}
			} catch (err) {
				this.emitError({
					extensionPath,
		...
		this.runtime.unregisterProvider = (name, extensionPath) => {
			...
			this.modelRegistry.unregisterProvider(name, extensionPath);
		};
	}
	...
	createContext(signalOverride?: AbortSignal): ExtensionContext {
		...
		return {
			...
			getRetryPolicy: () => {
				runner.assertActive();
				if (!runner.getRetryPolicyFn) {
					throw new Error("Host retry policy snapshot is not bound");
				}
				return runner.getRetryPolicyFn();
			},
		};
	}
