# @mizuikki/pi-agent-core

Stateful agent with tool execution and event streaming. Built on `@mizuikki/pi-ai`.

## Installation

```bash
npm install @mizuikki/pi-agent-core
```
...
```typescript
declare module "@mizuikki/pi-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@mizuikki/pi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@mizuikki/pi-agent-core";
