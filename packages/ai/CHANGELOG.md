...
- Fixed named tool selection for OpenAI Codex Responses, multi-line SSE parsing for pi-messages, and malformed Radius OAuth token response handling including refresh-token fallback when the response omits a rotated refresh token.
