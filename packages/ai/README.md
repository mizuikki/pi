...
```bash
npm install @mizuikki/pi-ai
```

TypeBox exports are re-exported from `@mizuikki/pi-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@mizuikki/pi-ai';
import { builtinModels } from '@mizuikki/pi-ai/providers/all';
...
```typescript
import { anthropicProvider } from '@mizuikki/pi-ai/providers/anthropic';
import { openaiProvider } from '@mizuikki/pi-ai/providers/openai';
import { openrouterProvider } from '@mizuikki/pi-ai/providers/openrouter';
import { amazonBedrockProvider } from '@mizuikki/pi-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@mizuikki/pi-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@mizuikki/pi-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@mizuikki/pi-ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@mizuikki/pi-ai';
...
```typescript
import { validateToolCall, type Tool } from '@mizuikki/pi-ai';
...
```typescript
import { builtinImagesModels } from '@mizuikki/pi-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@mizuikki/pi-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@mizuikki/pi-ai';
import { openAICompletionsApi } from '@mizuikki/pi-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@mizuikki/pi-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@mizuikki/pi-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@mizuikki/pi-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@mizuikki/pi-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@mizuikki/pi-ai';
import { anthropicProvider } from '@mizuikki/pi-ai/providers/anthropic';
import { openaiProvider } from '@mizuikki/pi-ai/providers/openai';
import { googleProvider } from '@mizuikki/pi-ai/providers/google';
...
- `@mizuikki/pi-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@mizuikki/pi-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@mizuikki/pi-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@mizuikki/pi-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@mizuikki/pi-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@mizuikki/pi-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@mizuikki/pi-ai/bedrock-provider';
...
```bash
npx @mizuikki/pi-ai login              # interactive provider selection
npx @mizuikki/pi-ai login anthropic    # login to specific provider
npx @mizuikki/pi-ai list               # list available providers
```
...
The legacy flow functions remain available via the `@mizuikki/pi-ai/oauth` entry point (`loginAnthropic`, `loginOpenAICodex`, `loginGitHubCopilot`, `refreshOAuthToken`, `getOAuthApiKey`); credential storage is the caller's responsibility there. New code should prefer the provider-owned `OAuthAuth` shown above — it composes with the credential store and gets locked auto-refresh for free.
...
```typescript
// Before
import { getModel, complete } from '@mizuikki/pi-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@mizuikki/pi-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@mizuikki/pi-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@mizuikki/pi-ai`.
