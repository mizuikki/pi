...
for (const pkg of workspacePackages) {
	for (const dependencyType of ["dependencies", "devDependencies"]) {
		...
		for (const [dependencyName, currentSpecifier] of Object.entries(dependencies)) {
			...
			// Local/prerelease versions must stay exact (not ^) so private forks do not
			// resolve against a nonexistent published range.
			const newSpecifier = version.includes("-") ? version : `^${version}`;
